...
for c in bin_coins:
    try:
        cn = c + 'BTC'
        btchist = bin_client.get_all_orders(symbol = cn)
    except:
        continue
    try:
        bnbhist = bin_client.get_all_orders(symbol = cn)
        cn = c + 'BNB'
    except:
        continue
...
bin_hist = pd.concat(bin_hist)

bin_hist = bin_hist[bin_hist.status == 'FILLED']



gdax_fills = gdax_client.get_orders()

flat_list = [item for sublist in gdax_fills for item in sublist]
gdax_hist = pd.DataFrame(flat_list)

gdax_fills = gdax_client.get_account_history('5c2b087a-271d-447e-9778-cbf2d53d997f')


gdax_hist = gdax_hist.rename(columns = {'created_at': 'date', 'size': 'amount'})

gdax_hist = gdax_hist[['date', 'paid', 'bought', 'amount', ]]


gdax_hist.bought = np.where(gdax_hist.side == 'buy', gdax_hist.product_id.str.split('-').str[0], gdax_hist.product_id.str.split('-').str[1])
gdax_hist.paid = np.where(gdax_hist.side == 'sell', gdax_hist.product_id.str.split('-').str[0], gdax_hist.product_id.str.split('-').str[1])
gdax_hist.cost = np.where(gdax_hist.side == 'buy', gdax_hist.usd_volume, gdax_hist.size)
gdax_hist.amt = np.where(gdax_hist.side == 'sell', gdax_hist.usd_volume, gdax_hist.size)
